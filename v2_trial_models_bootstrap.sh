...
while [ $run_arg -gt 1 ]; do
	...
	while [ $trial -lt $((n_trials+1)) ]; do
		if [ "$components" = "1" ]; then
			...
			python /home/pulsar/scratch/vonmises/generate_input.py -f $max_amp -n 1 >> paas.m 
		fi
	...
	#check the amplitudes of the von Mises components. If one is less than 1% of the profile max, end 
	#the routine and delete the last output file



	#after all $n_trials models have been compiled for the current number of component, take the 
	#lowest chi-square model and use a p-value test to check whether the <data> minus <model> 
	#residuals are likely to come from a Gaussian distribution (i.e. the residual is 
	#consistent with Gaussian noise). If so, stop the loop. Done using a Shapiro-Wilk test
	#
	#This becomes a problem when fitting models to extremely high S/N profiles, so as a 
	#second criterion, if the maximum value of the residual is less than 1% of the peak 
	#amplitude of the profile, stop the loop. #NOTE didn't do this
	#



	cat output_"$components".dat | awk '{print $NF,$0}' | sort -gr | cut -f2- -d' ' | tail -n1 | awk '{$NF=""; print $0}' | sed 's/ /\n/3; P; D' > paas.m

	small_components=$(cat paas.m | grep -v log | awk -v var="$max_amp" '$3<(var/100)' | wc -l)

	if [  "$small_components" -gt "1" ]; then
		run_arg=1
		rm output_"$components".dat
	else
		paas -L -r paas.m $archive
		run_arg=$(python /home/pulsar/scratch/vonmises/shapiro_wilks_test.py -f paas.txt)
	fi
