...
while [ $run_arg -gt 1 ]; do
	...
	while [ $trial -lt $((n_trials+1)) ]; do
		...
		#converged). If so,  write the paramters from paas.m (stripped of the line break) and the 
		#chi-squared of the model to the output file, then increase the trial counter
		if [ -e paas.txt ]; then
	...
	paas -L -r paas.m J*
	run_arg=$(python3.12 /home/pulsar/scratch/vonmises/decompose_vonMises/shapiro_wilks_test.py -f paas.txt)
